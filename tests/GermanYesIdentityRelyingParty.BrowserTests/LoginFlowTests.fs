...
module BrowserTests =
    ...
    module Steps =
        ...
        let teardown ctx = task {
            let! ctx = ctx
            ctx.Browser.CloseAsync().GetAwaiter().GetResult()
            return ctx
        }

        let initiateLogin ctx = task {
            let! ctx = ctx
        ...
        let confirmTwoFactorAuth ctx = task {
            let! ctx = ctx
        ...
        let declineSecondFactorAuth ctx = task {
            let! ctx = ctx
  ...
  "email": "test001@platform.yes.com"
    ...
    [<Fact>]
    let ``First time successful login`` () = task {
        let! ctx =
    ...
    [<Fact>]
    let ``Decline second factor auth`` () = task {
        let! ctx =
            setup "http://localhost:3000"
            |> initiateLogin
            |> cookieConsent
